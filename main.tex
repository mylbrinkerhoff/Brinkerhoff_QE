...
Within each LD, the different acoustic measures are assigned a coefficient indicating the measure has strength on the LD. When evaluating which measures had the greatest strength, you take the absolute coefficient value and look for the largest coefficients. This evaluation produced the results reported in Table~\ref{tab:LDA} (only the three strongest measures are shown for each LD).
\begin{table}[!h]
    \centering
    \caption{Acoustic measures with the highest absolute coefficient values for the linear discriminant functions.}
    \label{tab:LDA}
    \begin{tabular}{lll}
	\lsptoprule
	LD1  & LD2 & LD3  \\
